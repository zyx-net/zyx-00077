...
按照以下步骤操作，可验证规则影响模拟器功能端到端生效：

#### 1. 访问模拟器页面
1. 启动开发服务器：`npm run dev`
2. 访问 http://localhost:5173/
3. 点击左侧导航菜单「规则影响模拟」（在「异常申诉」和「规则配置」之间）
4. 进入模拟器列表页面，显示所有已创建的方案

#### 2. 创建模拟方案
1. 点击「创建方案」按钮
2. 输入方案名称（如「测试方案-放宽迟到」）和描述
3. 从下拉框选择一个已导入的批次（需先在「数据导入」页面导入批次数据）
4. 点击「创建」，自动进入编辑视图
5. 验证：页面显示四个参数滑块，默认值为当前规则配置

#### 3. 调整参数并运行模拟
1. 拖动「迟到宽限时间」滑块，从 10 分钟调整到 30 分钟
2. 拖动「早退阈值」滑块，从 0 分钟调整到 10 分钟
3. 拖动「跨日班最大时长」滑块，从 12 小时调整到 14 小时
4. 拖动「重复打卡窗口」滑块，从 5 分钟调整到 10 分钟
5. 点击「运行模拟」按钮
6. 验证：
   - 显示「正在运行模拟...」加载状态
   - 模拟完成后显示统计卡片：原异常数、模拟后异常数、净变化、模拟耗时
   - 显示差异明细列表，按类型分组（新增/移除/修改）
   - 显示按员工分组的差异详情，支持展开/折叠
   - 显示按异常类型的差异统计
...
3. 重新进入「规则影响模拟」页面
4. 找到刚才的方案，点击「编辑」
5. 验证：参数值保持为调整后的值，草稿已自动保存

#### 5. 应用方案
1. 在编辑视图中，确认模拟结果符合预期
2. 点击「应用方案」按钮
3. 确认弹窗显示预期影响（异常变化数量、受影响员工数）
4. 点击「确认应用」
5. 验证：
   - 方案状态变为「已应用」
   - 右上角 Toast 提示「方案应用成功」
   - 进入「规则配置」页面，确认规则已更新为模拟参数
   - 进入「异常分析」页面，重新选择批次，确认异常已按新规则重新计算

#### 6. 撤销应用
1. 在模拟器列表中找到「已应用」状态的方案
2. 点击「查看」进入详情视图
3. 点击「撤销应用」按钮
4. 确认弹窗显示恢复后的规则参数
5. 点击「确认撤销」
6. 验证：
   - 方案状态变为「已撤销」
   - 右上角 Toast 提示「撤销成功，规则已恢复」
   - 进入「规则配置」页面，确认规则已恢复到应用前的版本
   - 进入「异常分析」页面，确认异常已按原规则重新计算

#### 7. 导出方案
1. 在模拟器列表页面，勾选一个或多个方案
2. 点击「导出选中」按钮（或点击方案操作列的「导出」）
3. 验证：浏览器下载 JSON 文件，文件名包含时间戳

#### 8. 导入方案（含冲突处理）
1. 在模拟器列表页面，点击「导入方案」按钮
2. 选择刚才导出的 JSON 文件
3. 场景 A - 同名方案冲突：
   - 如果导入的方案名称已存在
   - 弹出冲突模态框，显示冲突类型：「同名方案」
...
4. 场景 B - 批次数据变化冲突：
   - 如果导入方案关联的批次数据已发生变化
   - 弹出冲突模态框，显示冲突类型：「批次数据已变化」
   - 选择「重新加载批次数据」
   - 验证：方案使用最新批次数据，需要重新运行模拟

#### 9. 删除方案
1. 在模拟器列表中找到「草稿」或「已就绪」状态的方案
2. 点击操作列的「删除」按钮
3. 确认删除弹窗
4. 验证：方案从列表中消失

#### 10. 权限控制验证
1. 以只读用户身份登录（或修改当前用户权限）
2. 进入「规则影响模拟」页面
3. 验证：
   - 页面顶部显示黄色「只读模式」提示条
   - 「创建方案」、「编辑」、「应用」、「删除」、「撤销」按钮均隐藏
   - 只能查看方案详情和模拟结果
...
- 9 个测试用例，覆盖 UI 层到 Store 层的状态同步
- 修正后状态同步验证
- revertCorrection 状态同步验证
- 多次修正撤回循环一致性验证
- 批量修正撤回统计数据验证
- 跨批次数据隔离验证
- **刚修正后立即撤回**：无需重新加载即可找到最新记录
- **批量修正后立即批量撤回**：所有异常状态同步更新
- **batch.stats 持久化回写**：撤回后从数据库读取 batch.stats 验证一致性
